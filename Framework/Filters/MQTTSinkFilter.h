...
namespace AtomIT
{
  class MQTTSinkFilter : public AdapterFilter
  {
  private:
    MQTT::MQTTClientWrapper   client_;

  protected:
    virtual PushStatus Push(const Message& message);
    
  public:
    MQTTSinkFilter(const std::string& name,
                   ITimeSeriesManager& manager,
                   const std::string& timeSeries);

    void SetBroker(const MQTT::Broker& broker)
    {
      client_.SetBroker(broker);
    }
