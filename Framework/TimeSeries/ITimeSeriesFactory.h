...
namespace AtomIT
{
  class ITimeSeriesFactory : public boost::noncopyable
  {
  public:
    ...
    virtual void ListManualTimeSeries(std::map<std::string, TimestampType>& target) = 0;

    virtual ITimeSeriesBackend* CreateManualTimeSeries(const std::string& name) = 0;
